...
$sql = "SELECT SUM( w.intake ) AS sum, u.nickname as name
		...
		WHERE w.intake_date >= DATE_ADD( NOW( ) , INTERVAL -6 HOUR )
		AND u.email = w.email
		AND u.idx = $u_idx;";
...
<div style="margin-top:15px;">
    <p style="margin-bottom:5px">총 <?php if($sum == 0) { echo 0; } else { echo $sum; } ?>mL 섭취</p>
	<p class="btn_wrap" style="margin: 5px 0 5px 0;">
		<a href="users_intake.php" class="bar">이전으로</a>
		<a href="" class="bar" style="font-weight: bold;">시간별</a>
		<a href="users_graph_day.php?u_idx=<?php echo $u_idx; ?>">일별</a>
    </p>
	<h1>사용자 '<?php echo $name;?>'님의 시간별 섭취량</h1>
<div class="vGraph">
	<ul>
        <?php
        // for($i = 1; $i <= $num; $i++){
        ...
        $sql = "SELECT DATE_FORMAT( a.date, '%m/%d %l' ) AS date, IFNULL( SUM( b.intake ) , 0 ) AS sum
				FROM (
				SELECT DATE_FORMAT( NOW( ) - INTERVAL( a.a ) HOUR , '%Y-%m-%d %H' ) AS DATE
				FROM (
				SELECT 0 AS a
				UNION ALL SELECT 1
				UNION ALL SELECT 2
				...
				LEFT JOIN (
				SELECT DATE_FORMAT( w.intake_date, '%Y-%m-%d %H' ) AS DATE, w.intake
				FROM water w, users u
				WHERE u.email = w.email and
				u.idx = $u_idx
				)b ON a.date = b.date
				GROUP BY a.date;";
        ...
        ?>
		<li>
			<span class="gTerm"><?php echo $array["date"]; ?>시　　</span>
