...
<div style="margin-top:15px;">
	...
	<h1 style="margin: 0 0 10px 0";>사용자 섭취량 순위</h1>
    <table>
        <tr class="title">
            <td>순위</td>
            <td>이메일</td>
        ...
        $sql = "select a.idx, a.email, a.nickname, a.gender, a.age, a.recent_visit, ifnull(b.sum, 0) as sum
				from (select *
				from users
				where del_yn='N') a
				left join
				(select u.email, sum(w.intake) as sum
				from users u, water w
				where del_yn = 'N'
				and u.email = w.email
				group by u.email) b
				on a.email = b.email
        ...
        <tr class="brd">
			...
			<td><?php $temp = $array["sum"]; echo (int)($temp/1000);?>L <?php echo $temp%1000; ?>mL</td>
			<td><a href="users_graph_hour.php?u_idx=<?php echo $array["idx"]; ?>">섭취량 그래프 보기</a></td>
        </tr>
        <?php  
            /* $i++; */
            /* paging */
    ...
    if($page <= 1){
    ?>
    <a href="users_intake.php?page=1">이전</a>
    <?php } else{ ?>
    <a href="users_intake.php?page=<?php echo ($page-1); ?>">이전</a>
    <?php };?>
    ...
    ?>
    <a href="users_intake.php?page=<?php echo $print_page; ?>"><?php echo $print_page; ?></a>
    <?php };?>
    ...
    if($page >= $total_page){
    ?>
    <a href="users_intake.php?page=<?php echo $total_page; ?>">다음</a>
    <?php } else{ ?>
    <a href="users_intake.php?page=<?php echo ($page+1); ?>">다음</a>
    <?php };?>
